...
# version1.0
"""
MHC-I Binding Prediction using IEDB Next-Generation API
...
def submit_mhc1_prediction(fasta_text, alleles, peptide_lengths=[8,11]):
    """
    Submit MHC-I prediction job to IEDB API
    
    Args:
        fasta_text: FASTA formatted sequences
        alleles: Comma-separated allele list (e.g., "HLA-A*02:01,HLA-A*01:01")
    ...
    payload = {
        "pipeline_title": "MHC-I Binding Prediction",
        "run_stage_range": [1, 1],
        "stages": [
            {
                "stage_number": 1,
                ...
                "input_parameters": {
                    ...
                    "predictors": [
                        {
                            "type": "binding",
                            "method": "netmhcpan_el"  # IEDB recommended method
                        }
                    ]
                }
            }
    ...
    print("Submitting MHC-I prediction job...")
...
def parse_and_save_results(results, output_file="mhc1_prediction.csv"):
    """
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: python3 mhc1_prediction.py <fasta_file> <alleles_file>")
        print("Example: python3 mhc1_prediction.py test.fasta alleles.txt")
        sys.exit(1)
    
    # Get file paths from command line
    fasta_file = sys.argv[1]
    alleles_file = sys.argv[2]
    output_file = "mhc1_prediction.csv"
    ...
    try:
        ...
        result_id, pipeline_id = submit_mhc1_prediction(
            fasta_text=fasta_text,
            alleles=alleles,
            peptide_lengths=[8,11]  # As requested
        )
        ...
        print(f"\n✓ MHC-I prediction completed successfully!")
